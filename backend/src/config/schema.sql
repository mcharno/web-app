...
-- ROM games table
-- One row per game. filenames[] holds every ROM file/directory on disk for that game.
-- title_key is derived from the filename(s) and used as a stable deduplication key.
-- Scraped metadata (title, description, etc.) is layered on top after scraping.
